...
TITLE OpenVPN-All-in-One by RUSViRTuE v1.0.0 [25.12.2022]
:START
:: -------------VARS--------------
:: Заполните значения переменных
:: IP-адрес сервера. Можно указать и локальный IP, и доменное имя, например, mydomain.com
...
chcp 866 >nul
:addsrvname
set /P SRV_NAME="Введите имя сервера (на английском и без пробелов): "
IF [%SRV_NAME%] == [] (
	cls
	CALL :EchoColor 4 "[ОШИБКА] ИМЯ СЕРВЕРА НЕ ДОЛЖНО БЫТЬ ПУСТЫМ"
	timeout /t 10
	cls
	goto addsrvname
	)
...
echo push "route %pushroute%">>"%SRV_FILE2%.ovpn"
...
echo ^</dh^>>>"%SRV_FILE2%.ovpn"
echo ^<crl-verify^>>>"%SRV_FILE2%.ovpn"
echo ^</crl-verify^>>>"%SRV_FILE2%.ovpn"
:: Окончание создания %SRV%.ovpn-файла
::-----------------------------------------

echo f|xcopy /y "%SRV_FILE1%.key" "%SRV_FILE2%.key" >nul
echo f|xcopy /y "%SRV_FILE1%.crt" "%SRV_FILE2%.crt" >nul
echo f|xcopy /y "%KEY_DIR%\ca.crt" "%KEY_DIR%\%SRV_NAME%\ca.crt" >nul
echo f|xcopy /y "%KEY_DIR%\ta.key" "%KEY_DIR%\%SRV_NAME%\ta.key" >nul
echo f|xcopy /y "%KEY_DIR%\dh%KEY_SIZE%.pem" "%KEY_DIR%\%SRV_NAME%\dh%KEY_SIZE%.pem" >nul
echo f|xcopy /y "%SRV_FILE2%.ovpn" "%OpenVPN_DIR%\config\%SRV_NAME%.ovpn" >nul
...
:CLIENT-CRT
set /P CLIENT_NAME="Введите имя клиента (на английском и без пробелов): "
IF [%CLIENT_NAME%] == [] (
	cls
	CALL :EchoColor 4 "[ОШИБКА] ИМЯ КЛИЕНТА НЕ ДОЛЖНО БЫТЬ ПУСТЫМ"
	timeout /t 10
	cls
	goto CLIENT-CRT
	)
...
for /f "tokens=1 delims=:" %%a in ('findstr /nc:"%w%" "%f_in%"') do (call :sum %%a)
del "%f_tmp%"
del "%f_in%"
